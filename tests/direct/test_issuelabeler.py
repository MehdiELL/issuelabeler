# Direct-mode unit tests for IssueLabeler.  Run: pytest tests/direct/ -v
import json

CONTRACT = "contracts/issuelabeler.py"
...
def test_submit_and_evaluate(direct_vm, direct_deploy, direct_alice):
    ...
    assert int(idx) == 0
    direct_vm.mock_llm(r".*JSON.*", json.dumps({"label": "bug", "score": 80, "reasoning": "ok"}))
    c.evaluate(0)
    item = c.get_item(0)
